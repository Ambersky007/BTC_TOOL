...
def scalp_entry():
    ...
    if sweep == "up_sweep" and price < zone.high:
        if sell_price > price:
            ...
            return
    qty_ratio = POSITION_RATIO
    if current_zone_score >= 70:
        qty_ratio = POSITION_RATIO
        strategy_suffix = "优质区"
    elif current_zone_score >= 50:
        qty_ratio = POSITION_RATIO * 0.5
        strategy_suffix = "试探区"
    else:
        return
    ...
    if USE_LOCAL_SIMULATION:
        ...
        while retry_count < MAX_RETRY:
            ...
            if success:
                ...
                try:
                    url = f"{BASE_URL}/fapi/v1/order"
                    # 尝试获取最近的一笔平仓单，实际生产环境应通过listenKey抓取ORDER_TRADE_UPDATE获取精确佣金
        ...
        if not cur_pos or cur_ep <= 0:
            ...
            # 🚀 实盘模式：走官方精准盈亏逻辑
                ...
                with lock:
                    off_pnl = official_unrealized_profit
                    off_margin = official_isolated_margin
                # 止损触发已经在 manage_position 处理，这里处理止盈与极值追踪
                # 由于实盘不依赖本地算价，极值仍需用WS最新价追踪，用于追踪止盈判断
                if cur_pos == "long":
            ...
            status_parts = []
            if max_profit_pct > 0:
                # ⚠️ 注意：这里的retracement由于混用WS极值和官方当前盈亏，可能极小概率有微毛刺，但为了极速兑现止盈这是最优解
                retracement = pr / max_profit_pct if max_profit_pct > 0 else 0
                time_since_high = time.time() - time_at_max_profit
                # 追踪止盈逻辑 (实盘与本地共用逻辑框架，但pr来源不同)
                if (USE_LOCAL_SIMULATION and pr <= 0.0006) or (not USE_LOCAL_SIMULATION and off_pnl <= 0):
            ...
            if now - last_holding_log_time >= 2:
                if USE_LOCAL_SIMULATION:
                    log(f"[本地持仓状态] {cur_pos} | 推算盈亏:{pr:.2%} | 最高:{max_profit_pct:.2%} | 状态面板: {' | '.join(status_parts)}")
                else:
                    with lock:
                        log(f"[实盘持仓状态-官方] {cur_pos} | 官方盈亏:{off_pnl:.2f}U (ROI:{pr:.2%}) | 极值最高:{max_profit_pct:.2%} | 状态面板: {' | '.join(status_parts)}")
                    ...
                    if pos_data.get("s") == SYMBOL:
                        ...
                        with lock:
                            if amt != 0:
                                ...
                                # 官方推送清仓，立即清除本地缓存的持仓与盈亏
                                position = None
                                entry_price = 0
                                actual_position_amt = QTY
                                official_unrealized_profit = 0.0
                                official_isolated_margin = 0.0
                        # 收到官方盈亏更新，放入交易队列触发风控判断
                        trade_queue.put("PROCESS")
            elif data.get("e") == "ORDER_TRADE_UPDATE":
                # 精确的订单成交与手续费扣减事件，可用于最精确的盈亏归因
                o = data.get("o", {})
...
if __name__ == "__main__":
    ...
    sync_position()
    start_ws()
    start_user_data_stream()  # 🚀 启动官方私有流
