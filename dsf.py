...
actual_position_amt = QTY  # 记录API模式下的实际持仓数量(兼容部分成交)
# 🔥🔥🔥 核心修改：仓位与资金管理 🔥🔥🔥
INITIAL_BALANCE = 10000.0  # 本地撮合模式初始模拟资金(USDT)
POSITION_RATIO = 0.3  # 开仓占总资金比例：30%
LOT_SIZE = 0.001  # BTCUSDT下单步长(精度)
...
# 🔥 核心修改2：优化15分钟趋势判定逻辑，增加震荡中性
def get_15m_trend():
    """最近1根15分钟收盘K线：阳线/十字星(震荡)允许多，阴线/十字星(震荡)允许空"""
    if not kline_15m_closed:
        return "neutral"  # 兜底
    last_k = kline_15m_closed[-1]
    # 极小实体判定：波动小于0.02%视为十字星/震荡
        ...
        for item in d:
            if item['symbol'] == SYMBOL:
                ...
                if amt != 0:  # 有持仓
                    ...
                    position = None
                    entry_price = 0
                    actual_position_amt = QTY  # 重置为默认下单量
                    log("[API持仓] None")
    except Exception as e:
        log(f"[持仓同步失败] {e}")
...
def send_market_order(side, reduce=False, qty=None):
    ...
    # 确定平仓数量：优先使用传入的qty，其次使用实际持仓数量，最后使用默认QTY
...
def check_fast_cancel():
    ...
    if USE_LOCAL_SIMULATION:
        ...
        try:
            ...
            return
        # 挂单价格与买一价差超过6个步长，撤单
        if abs(local_order_price - best_bid) > 6 * PRICE_STEP:
            cancel_order()
    else:
...
def v18_entry_logic():
    ...
    # 🔥 核心修改：使用15分钟大趋势过滤 (优化为非此即彼逻辑)
    trend_15m = get_15m_trend()
    allow_long = trend_15m in ["up", "neutral"]  # 阳线或震荡可做多
    allow_short = trend_15m in ["down", "neutral"]  # 阴线或震荡可做空
    # 盘口
    bias = orderbook_bias()
    # KDJ
    ...
    # 🔥 核心修改：放开 J 值绝对区间限制(删除 J<=50)，只看拐头方向和力度！
    ...
    long_ok = long_cond_trend and long_cond_bias and long_cond_shadow and long_cond_volume and long_cond_kdj_dir and long_cond_kdj_force and long_cond_flow
    # ============ 开空条件判定 ============
    ...
    short_ok = short_cond_trend and short_cond_bias and short_cond_shadow and short_cond_volume and short_cond_kdj_dir and short_cond_kdj_force and short_cond_flow
    # 构造日志
    if now - last_entry_log_time >= 2:
        log_long = (
            f"[开多条件] 15m趋势阳/震:{'✅' if long_cond_trend else '❌'} | "
        ...
        log_short = (
            f"[开空条件] 15m趋势阴/震:{'✅' if short_cond_trend else '❌'} | "
    ...
    try:
        ...
        if data.get("e") == "kline":
            ...
            if k_data["x"]:  # 只有K线收盘时才记录，确保数据完整不重复
                ...
                elif k_data["i"] == "15m":  # 🔥 新增：捕获15分钟K线
                    kline_15m_closed.append(new_k)
                    if len(kline_15m_closed) > 2:
                        kline_15m_closed.pop(0)
    except Exception as e:
        # 解析异常也更新心跳，避免误判断连
        last_ws_time = time.time()
...
if __name__ == "__main__":
    ...
    log(f"        V19 趋势共振版 启动成功")
    log(f"        当前运行模式: {mode_str}")
    log(f"        大趋势: 15分钟K线定方向 | 仓位控制: 总资金 {POSITION_RATIO * 100:.0f}%")
    log("======================================")
    sync_binance_time()  # 同步币安服务器时间
    fetch_initial_klines()  # 🔥 核心：启动时主动拉取历史K线填补空白，防止开局死锁
