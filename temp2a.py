            ...
            if max_profit_pct > 0:
                ...
                if (USE_LOCAL_SIMULATION and pr < MIN_ROI_COVER_FEE) or (not USE_LOCAL_SIMULATION and off_pnl < 0):
                    ...
                    if max_profit_pct < MIN_ROI_COVER_FEE * 1.2:
                        ...
                        if is_trend_holding:
                            trail_tp = (time_since_high >= 12) or (retracement <= 0.70)  # 趋势单：回撤30%或12秒未新高
                            trail_reason = "趋势单-回撤30%或12秒未新高"
                        else:
                            trail_tp = (time_since_high >= 5) or (retracement <= 0.85)  # 震荡单：回撤15%或5秒未新高
