...
def scalp_entry():
    ...
    if min_close < zone.low:
        ...
        buy_price = normalize_price(buy_price)
    # ================= 3. 15分钟大趋势绝对过滤 =================
    trend_15m = detect_trend_state()
    if trend_15m in ["strong_long", "weak_long"]:
        short_allowed = False
    if trend_15m in ["strong_short", "weak_short"]:
        long_allowed = False
    # ================= 4. 上一根K线阴阳过滤 =================
    with data_lock:
        if len(kline_1m_closed) >= 1:
            last_k = kline_1m_closed[-1]
            is_last_yang = last_k['close'] > last_k['open']
            is_last_yin = last_k['close'] < last_k['open']
            if is_last_yang: long_allowed = False
            if is_last_yin: short_allowed = False
    # ================= 5. 未收盘K线动量盲区过滤 =================
    with data_lock:
        if current_forming_kline and atr > 0:
            fk = current_forming_kline
            forming_body = abs(fk['close'] - fk['open'])
            is_forming_bull = fk['close'] > fk['open']
            is_forming_bear = fk['close'] < fk['open']
            if forming_body > atr * 0.8:
                if is_forming_bull: short_allowed = False
                if is_forming_bear: long_allowed = False
    # ================= 区间评分与仓位管理 =================
...
def on_message(ws, msg):
    global orderbook, raw_price, current_price, last_trade_side, last_ws_time, last_aggTrade_time, kline_1m_closed, kline_15m_closed, mark_price, prev_price_v23, current_forming_kline
...
def ws_watchdog():
    ...
    while True:
        ...
        now = time.time()
        # 1. 检查底层心跳断流
        if now - last_ws_time > 15:
            log("🚨 WS底层心跳断流超过15秒，主动断开触发重连！")
            try:
                if ws_app: ws_app.close()
            except:
                pass
            last_ws_time = now
        # 2. 🚀 核心修复：检查业务数据断流（心跳在但没行情）
        elif now - last_aggTrade_time > 10:
            log("🚨 WS心跳正常，但aggTrade行情断流超过10秒！可能TCP假死，强制重连！")
            try:
                if ws_app: ws_app.close()
            except:
                pass
            last_aggTrade_time = now  # 重置以防频繁触发
