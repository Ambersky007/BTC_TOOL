...
trade_buffer = []
current_forming_kline = None  # 记录当前未收盘的1分钟K线
FAST_CANCEL_SEC = 2
cooldown_until = 0
COOLDOWN_SEC = 2
# V23 核心参数
...
trade_queue = queue.Queue(maxsize=200)  # 限制最大堆积量
max_profit_pct = 0.0
...
stop_watch_count = 0  # 🚀 修复新增：V24 止损状态机观察期计数器
# 🚀 V24 新增：开仓时记录的上下文与状态机计数
entry_trend_state = "weak"
entry_zone_score_log = 0
current_5m_kline = None  # 🚀 新增：记录当前未收盘的5分钟K线，用于动态趋势过滤
...
def get_dynamic_zone_validity(atr):
    # 🚀 阈值下调：1分钟ATR达到30以上即视为快行情，轨道存活时间缩短
    if atr < 30:
        return 180
    elif atr < 60:
        return 90  # 原120缩短，大波动轨道刷新要快
    else:
        return 45  # 极速行情，轨道存活时间极短，防止用过期轨道开仓
...
def compute_atr(klines, period=20):  # 🚀 默认周期从7改为20，适应大波动
...
# ======================
def sync_position():
    global position, entry_price, active_order, actual_position_amt, official_unrealized_profit, official_isolated_margin, entry_atr
    if USE_LOCAL_SIMULATION:
        if local_position:
...
def send_market_order(side, reduce=False, qty=None):
    ...
    global active_order, actual_position_amt, entry_atr
    ...
    if USE_LOCAL_SIMULATION:
        if reduce:
            ...
            with data_lock:
                entry_atr = compute_atr(kline_1m_closed, 7) if len(kline_1m_closed) >= 7 else 0.0
            log(f"[本地市价开仓] 记录开仓ATR: {entry_atr:.1f}")
        return True
    for i in range(3):
        try:
            p = {
...
def check_order_filled():
    ...
    global partial_filled_flag, actual_position_amt, entry_atr
    if USE_LOCAL_SIMULATION:
        if not local_active_order: return
        ...
        if filled:
            if is_reduce:
                ...
                with data_lock:
                    entry_atr = compute_atr(kline_1m_closed, 7) if len(kline_1m_closed) >= 7 else 0.0
                log(f"🎉 [本地限价单成交] {side} | 成交价:{order_p} | 数量:{actual_position_amt} | 记录开仓ATR:{entry_atr:.1f}")
        return
    if not active_order: return
    try:
        p = {"symbol": SYMBOL, "orderId": active_order, "timestamp": get_timestamp(), "recvWindow": 5000}
        ...
        if d.get("status") == "FILLED":
            ...
            if is_reduce:
                ...
                with data_lock:
                    entry_atr = compute_atr(kline_1m_closed, 7) if len(kline_1m_closed) >= 7 else 0.0
                log(f"🎉 [API限价单成交] {d.get('side')} | 成交价:{d.get('avgPrice')} | 记录开仓ATR:{entry_atr:.1f}")
                sync_position()
...
def detect_trend_state():
    ...
    with data_lock:
        ...
        # 🚀 修复核心：深拷贝最后6根K线，防止在锁外计算时被WS线程pop导致索引越界崩溃
        k_copy = [k.copy() for k in kline_1m_closed[-6:]]
        atr_now = compute_atr(kline_1m_closed, 7)
        atr_prev = compute_atr(kline_1m_closed[:-1], 7) if len(kline_1m_closed) > 7 else atr_now
    # 结构 (使用深拷贝的 k_copy)
    hh = k_copy[-1]['high'] > k_copy[-2]['high'] > k_copy[-3]['high']
    hl = k_copy[-1]['low'] > k_copy[-2]['low'] > k_copy[-3]['low']
    lh = k_copy[-1]['high'] < k_copy[-2]['high'] < k_copy[-3]['high']
    ll = k_copy[-1]['low'] < k_copy[-2]['low'] < k_copy[-3]['low']
    # 动量
    bodies = [abs(x['close'] - x['open']) for x in k_copy[-5:]]
...
def is_multi_tf_trending():
    with data_lock:
        ...
        # 至少2根K线实体大于0.6倍ATR，确认为强势启动
        strong = [abs(k['close'] - k['open']) > atr * 0.6 for k in last_3]
        if (all_bull or all_bear) and sum(strong) >= 2:
            return "bull" if all_bull else "bear"
        return None


# 🚀🚀🚀 新增：量价共振确认 (配合趋势启动锁使用) 🚀🚀🚀
def is_volume_price_resonance():
    with data_lock:
        if len(kline_1m_closed) < 5:
            return False
        vols = [k['volume'] for k in kline_1m_closed[-5:]]
        avg_vol = sum(vols[:-1]) / 4
        last = kline_1m_closed[-1]
        body = abs(last['close'] - last['open'])
        atr = compute_atr(kline_1m_closed, 7)
        if atr <= 0: return False
        # 量能放大1.5倍且实体大于0.7倍ATR
...
def detect_range():
    ...
    with data_lock:
        ...
        atr = compute_atr(kline_1m_closed, 20)  # 🚀 ATR周期改为20
        if atr <= 0 or range_width <= 0:
            current_zone_score = 0
            return False
        # =========================
        # 1️⃣ 区间宽度评分（🚀 大幅放宽上限，适配1000U宽幅震荡）
        # =========================
        if atr * 2.0 <= range_width <= atr * 15.0:  # 原 1.5~6 改为 2.0~15.0
            score1 = 20
        elif range_width < atr * 2.0:
            score1 = 15
        else:
            score1 = 10
        # =========================
...
def log_entry_conditions():
    ...
    with data_lock:
        ...
        atr = compute_atr(kline_1m_closed, 20) if kline_len >= 20 else 0.0  # 🚀 周期改为20，条件改为20
        cond1_5k = False
    ...
    cond2_kline = kline_len >= 20  # 🚀 基础K线根数要求同步改为20
    cond2_atr = atr > 0
    zone_width = 0.0
    if liquidity_zone:
    ...
    if liquidity_zone and raw_price and raw_price > 0 and atr > 0:
        # 🚀🚀🚀 核心修改：放大止损乘数，让开仓即带200U左右的安全垫 🚀🚀🚀
        if atr < 50:
            atr_multiplier = 2.0  # 原0.25 -> 2.0 (预留约100U止损空间)
        elif atr < 120:
            atr_multiplier = 2.5  # 原0.35 -> 2.5 (预留约150U止损空间)
        else:
            atr_multiplier = 3.0  # 原0.5 -> 3.0 (预留200U+止损空间)
    ...
    # ================= V24 新增：四大过滤器雷达 =================
    cond7_trend = detect_trend_state()
    cond7_long_allowed = cond7_trend not in ["strong_short", "weak_short"]
    cond7_short_allowed = cond7_trend not in ["strong_long", "weak_long"]
    cond8_last_k = "neutral"
    with data_lock:
        if kline_len >= 1:
            ...
            if last_k['close'] > last_k['open']:
                ...
                cond8_last_k = "yin"
    cond8_long_allowed = cond8_last_k != "yang"
    cond8_short_allowed = cond8_last_k != "yin"
    cond9_forming = "neutral"
    with data_lock:
        if current_forming_kline and atr > 0:
            fk = current_forming_kline
            fb = abs(fk['close'] - fk['open'])
            if fb > atr * 0.8:
                if fk['close'] > fk['open']:
                    cond9_forming = "bull"
                elif fk['close'] < fk['open']:
                    cond9_forming = "bear"
    cond9_long_allowed = cond9_forming != "bear"
    cond9_short_allowed = cond9_forming != "bull"
    cond10_breakout = "none"
    with data_lock:
        if kline_len >= 3 and liquidity_zone and atr > 0:
            last_3_closes = [k['close'] for k in kline_1m_closed[-3:]]
            min_close = min(last_3_closes)
            max_close = max(last_3_closes)
            breakout_tolerance = max(PRICE_STEP * 2, atr * 0.5)
            if max_close > liquidity_zone.high:
                if (max_close - liquidity_zone.high) >= breakout_tolerance:
                    cond10_breakout = "up_real"
                else:
                    cond10_breakout = "up_fake"
            elif min_close < liquidity_zone.low:
                if (liquidity_zone.low - min_close) >= breakout_tolerance:
                    cond10_breakout = "down_real"
                else:
                    cond10_breakout = "down_fake"
    cond10_short_allowed = cond10_breakout != "up_real"
    cond10_long_allowed = cond10_breakout != "down_real"
    all_long = is_range_code and cond2 and cond3 and cond4_long and cond5_long and cond6 and cond7_long_allowed and cond8_long_allowed and cond9_long_allowed and cond10_long_allowed
    all_short = is_range_code and cond2 and cond3 and cond4_short and cond5_short and cond6 and cond7_short_allowed and cond8_short_allowed and cond9_short_allowed and cond10_short_allowed
    log(
        f"[V24开仓雷达] "
        ...
        f"7.15m趋:{'✅' if (cond7_long_allowed and cond7_short_allowed) else '⚠️'}({cond7_trend}) | "
        f"8.K线阴阳:{'✅' if (cond8_long_allowed and cond8_short_allowed) else '⚠️'}({cond8_last_k}) | "
        f"9.未收盘:{'✅' if (cond9_long_allowed and cond9_short_allowed) else '⚠️'}({cond9_forming}) | "
        f"10.破轨:{'✅' if (cond10_long_allowed and cond10_short_allowed) else '⚠️'}({cond10_breakout}) | "
...
def scalp_entry():
    ...
    atr = 0.0
    with data_lock:
        if len(kline_1m_closed) >= 20:  # 🚀 根数改为20
            atr = compute_atr(kline_1m_closed, 20)  # 🚀 周期改为20
            # 🚀🚀🚀 核心删除：移除 if atr > PRICE_STEP * 20: return 🚀🚀🚀
            # 大波动周期ATR必然高，原逻辑会直接掐断所有大波动开仓机会
    ...
    # ================= 新增：趋势启动锁 (优先级最高) =================
    trend_dir_lock = is_multi_tf_trending()
    if trend_dir_lock and is_volume_price_resonance():
        return
    # ================= 新增：微趋势过滤 (1分钟连续4根) =================
    micro_trend = is_micro_trend()
    long_allowed = True
    short_allowed = True
    if micro_trend == "bear": long_allowed = False
    if micro_trend == "bull": short_allowed = False
    # ================= 新增：动态趋势过滤 (5分钟+1分钟共振) =================
    trend_dynamic = detect_dynamic_trend()
    if trend_dynamic == "bull": short_allowed = False
    if trend_dynamic == "bear": long_allowed = False
    # ================= 原有逻辑整合 =================
    price = raw_price
    zone = liquidity_zone
    bias = orderbook_bias()
    ...
    with data_lock:
        ...
        max_close = max(last_3_closes)
    # 破轨容忍度
    breakout_tolerance = max(PRICE_STEP * 2, atr * 0.5) if atr > 0 else PRICE_STEP * 2
    # ------------------ 1. 做空（上轨）破轨评估 ------------------
    if max_close > zone.high:
        breakout_dist = max_close - zone.high
        if breakout_dist >= breakout_tolerance:
            is_volume_breakout = False
            is_no_retracement = False
            with data_lock:
                if current_forming_kline and atr > 0:
                    forming_body = abs(current_forming_kline['close'] - current_forming_kline['open'])
                    is_volume_breakout = forming_body > atr * 0.8
                is_no_retracement = raw_price > (max_close - atr * 0.1)
            if is_volume_breakout and is_no_retracement:
                short_allowed = False
        sell_price = max(zone.high + 5, max_close)
    ...
    if min_close < zone.low:
        breakout_dist = zone.low - min_close
        if breakout_dist >= breakout_tolerance:
            is_volume_breakout = False
            is_no_retracement = False
            with data_lock:
                if current_forming_kline and atr > 0:
                    forming_body = abs(current_forming_kline['close'] - current_forming_kline['open'])
                    is_volume_breakout = forming_body > atr * 0.8
                is_no_retracement = raw_price < (min_close + atr * 0.1)
            if is_volume_breakout and is_no_retracement:
                long_allowed = False
        buy_price = min(zone.low - 5, min_close)
    ...
    if price <= zone.low + PRICE_STEP * 5:
        if zone.touch_count >= 0 and (
            ...
            if long_allowed:
                current_entry_strategy = f"V23低吸-大波动版({strategy_suffix}评分{current_zone_score})"
    ...
    if price >= zone.high - PRICE_STEP * 5:
        if zone.touch_count >= 0 and (bias != "bull" or detect_absorption() or exhaustion) and last_trade_side != "buy":
            if short_allowed:
                current_entry_strategy = f"V23高抛-大波动版({strategy_suffix}评分{current_zone_score})"
...
def dynamic_stop_filter(direction, current_price, ep, entry_atr_val):
    ...
    if not current_price or not ep or entry_atr_val <= 0:
        return False
    if direction == "long":
        reverse_offset = ep - current_price
    else:
        reverse_offset = current_price - ep
    ratio = reverse_offset / entry_atr_val
    # 🚀 状态机阈值全面上调：3.5->7.0 / 2.5->5.0 / 2.0->3.5
    # 阶段3：极端保护 (无条件止损)
    if ratio > 7.0:
        log(f"[止损状态机] 强制止损触发！ATR极度爆炸(7.0倍) | 反向偏移:{reverse_offset:.1f}")
        stop_watch_count = 0
        return True
    # 阶段2：确认趋势反转（5.0倍且持续2个周期）
    if ratio > 5.0 and stop_watch_count >= 3:
        log(f"[止损状态机] 趋势确认止损触发！ATR放大5.0 + 持续{stop_watch_count}次 | 反向偏移:{reverse_offset:.1f}")
        stop_watch_count = 0
        return True
    # 阶段1：观察期 (3.5倍以上开始计次)
    if ratio > 3.5:
        stop_watch_count += 1
        log(f"[止损状态机] 进入观察期！ATR放大3.5 | 计次:{stop_watch_count} | 反向偏移:{reverse_offset:.1f}")
    else:
        stop_watch_count = 0
    return False
# 🚀🚀🚀 修复重构：止损激活过滤器（基于开仓ATR反向放大检测） 🚀🚀🚀
def stop_activation_filter(direction, current_price, ep, entry_atr_val):
    if not current_price or not ep or entry_atr_val <= 0:
        return False
    if direction == "long":
        reverse_offset = ep - current_price
        ...
        reverse_offset = current_price - ep
        # 🚀 核心修改：2.5倍放宽至 5.5倍 (1分钟ATR若为40U，5.5倍=220U，刚好扛住一根5分钟插针)
    if reverse_offset > entry_atr_val * 5.5:
        log(f"[止损激活检测] 反向波动放大触发！反向偏移:{reverse_offset:.1f} > 开仓ATR*5.5:{entry_atr_val * 5.5:.1f}")
        return True
    log(f"[止损激活检测] 未触发。反向偏移:{reverse_offset:.1f} <= 开仓ATR*5.5:{entry_atr_val * 5.5:.1f}")
    return False
    ...
    if USE_LOCAL_SIMULATION:
        ...
        if ep > 0:
            ...
            if pnl <= -0.05:
                ...
                return True
            if pnl <= -SCALP_SL:
                # 🚀 修复重构：使用开仓时的ATR和反向偏移判定
                if stop_activation_filter(pos, price, ep, entry_atr):
                    close_position("V23硬止损(ATR反向放大)")
                    return True
                else:
        ...
        if off_margin > 0:
            ...
            if roi <= -SCALP_SL:
                ...
                if stop_activation_filter(pos, price, entry_price, entry_atr):
                    close_position("V23硬止损(官方盈亏-ATR反向放大)")
                    return True
                else:
...
# ======================
def on_open(ws):
    global reconnect_delay, last_aggTrade_time, last_ws_time
    reconnect_delay = 5
    # 🚀 修复：重置时间戳，防止刚连接就被看门狗误杀
    ...
    time.sleep(0.5)
    sync_balance()
    # 🚀 修复：depth5@100ms 数据量太大且太费性能，改为 depth10@500ms
    params = [
        f"{WS_SYMBOL}@aggTrade", f"{WS_SYMBOL}@markPrice", f"{WS_SYMBOL}@depth5@500ms",   #如果网络允许，改回100ms
...
def on_message(ws, msg):
    global orderbook, raw_price, current_price, last_trade_side, last_ws_time, last_aggTrade_time, kline_1m_closed, kline_15m_closed, mark_price, prev_price_v23, current_forming_kline, current_5m_kline
    last_ws_time = time.time()
    try:
        data = json.loads(msg)
        # 兼容单流和组合流
        ...
        if data.get("e") == "markPriceUpdate":
            ...
            try:
                trade_queue.put_nowait("PROCESS")
            except queue.Full:
                pass  # 队列满时直接丢弃，防止卡死WS线程
            return
        # ================= 2. 深度流 =================
        # 兼容 depthUpdate 和 部分深度流(如 depth5@500ms，无 "e" 字段)
        ...
        # 深度流不触发 PROCESS，交由 aggTrade 触发，大幅降低处理频率
        # ================= 3. AggTrade (最核心行情) =================
        ...
        if data.get("e") == "kline":
            k_data = data["k"]
            if not k_data["x"]:  # 未收盘K线
                # 未收盘K线无需加锁，仅主线程读取时存在微弱延迟，可接受
                if k_data["i"] == "1m":
                    current_forming_kline = {
                ...
                if k_data["i"] == "1m":
                    current_forming_kline = None
                elif k_data["i"] == "5m":
                    current_5m_kline = None
                new_k = {
...
def ws_watchdog():
    global last_ws_time, ws_app, last_aggTrade_time
    while True:
        time.sleep(5)
        now = time.time()
        if now - last_ws_time > 20:
            log("🚨 WS底层心跳断流超过20秒，主动断开触发重连！")
            try:
                if ws_app: ws_app.close()
            except:
                pass
            last_ws_time = now
        # 🚀 核心修复：aggTrade 断流超过 30 秒，说明高频流已死，必须重连！
        elif now - last_aggTrade_time > 30:
            log(f"🚨 aggTrade行情断流{now - last_aggTrade_time:.0f}秒！高频数据死亡，强制重连...")
            try:
                if ws_app: ws_app.close()
            except:
                pass
            last_ws_time = now
            last_aggTrade_time = now  # 重置时间，防止重复触发
...
def start_user_data_stream():
    ...
    def on_user_data_message(ws, msg):
        global official_unrealized_profit, official_isolated_margin, position, entry_price, actual_position_amt, entry_atr
        try:
            data = json.loads(msg)
