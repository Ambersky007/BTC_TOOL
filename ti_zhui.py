...
def check_order_timeout():
    if USE_LOCAL_SIMULATION:
        if local_active_order and time.time() - local_order_time > ORDER_TIMEOUT: cancel_order()
    else:
        if active_order:
...
def close_position(reason):
    ...
    if entry_time > 0:
        ...
        record = {
            ...
            "开单策略": current_entry_strategy,  # 🔥 新增策略记录
...
def v18_entry_logic():
    global cooldown_until, loss_count, last_entry_log_time, current_entry_strategy
    now = time.time()
    if USE_LOCAL_SIMULATION:
    ...
    # 🔥 核心修复：使用15分钟趋势过滤大方向，解决追单总是错方向的问题
    ...
    pb_short_cond_volume_1 = not (is_3_drop and (k0['volume'] < k1['volume'] < k2['volume']))
    ...
    # 引擎二：追单策略
    is_2_rise = (k0['close'] > k0['open']) and (k1['close'] > k1['open'])
    is_vol_surge_up = k0['volume'] > avg_vol_3 * 1.2 or k1['volume'] > avg_vol_3 * 1.2
    is_vol_sustain_up = k0['volume'] >= k1['volume'] * 0.6
    # 修复5：放宽追单盘口与J值
    chase_long_ok = (
            allow_long and is_2_rise and is_vol_surge_up and is_vol_sustain_up and
            bias != "bear" and not is_long_upper_shadow(k0) and j0 >= 50
    )

    is_2_drop = (k0['close'] < k0['open']) and (k1['close'] < k1['open'])
    is_vol_surge_down = k0['volume'] > avg_vol_3 * 1.2 or k1['volume'] > avg_vol_3 * 1.2
    is_vol_sustain_down = k0['volume'] >= k1['volume'] * 0.6
    chase_short_ok = (
            allow_short and is_2_drop and is_vol_surge_down and is_vol_sustain_down and
            bias != "bull" and not is_long_lower_shadow(k0) and j0 <= 50
    )

    long_ok = pullback_long_ok or chase_long_ok
    short_ok = pullback_short_ok or chase_short_ok

    # 修复8：更新日志字符串
    if now - last_entry_log_time >= 2:
        log_long = (
            f"[开多条件] 15m阳/震:{'✅' if allow_long else '❌'} | J偏弱≤60:{'✅' if j_oversold else '❌'} | "
            ...
            f"【追单】2连阳:{'✅' if is_2_rise else '❌'} 放量:{'✅' if is_vol_surge_up else '❌'} 量持续:{'✅' if is_vol_sustain_up else '❌'} 盘口非空:{'✅' if bias != 'bear' else '❌'} J≥50:{'✅' if j0 >= 50 else '❌'} 无上影:{'✅' if not is_long_upper_shadow(k0) else '❌'}"
        )
        log_short = (
            f"[开空条件] 15m阴/震:{'✅' if allow_short else '❌'} | J偏强≥40:{'✅' if j_overbought else '❌'} | "
            f"【剃头皮】盘口非多:{'✅' if pb_short_cond_bias else '❌'} 无下影:{'✅' if pb_short_cond_shadow else '❌'} 非缩量跌:{'✅' if pb_short_cond_volume else '❌'} J降:{'✅' if pb_short_cond_kdj_dir else '❌'} J力度:{'✅' if pb_short_cond_kdj_force else '❌'} 卖主导:{'✅' if pb_short_cond_flow else '❌'} | "
            f"【追单】2连阴:{'✅' if is_2_drop else '❌'} 放量:{'✅' if is_vol_surge_down else '❌'} 量持续:{'✅' if is_vol_sustain_down else '❌'} 盘口非多:{'✅' if bias != 'bull' else '❌'} J≤50:{'✅' if j0 <= 50 else '❌'} 无下影:{'✅' if not is_long_lower_shadow(k0) else '❌'}"
        )
        log(log_long);
        log(log_short);
        log("-" * 120)
...
def process_price():
    ...
    try:
        ...
        with lock:
            if USE_LOCAL_SIMULATION:
                cur_pos = local_position; cur_ep = local_entry_price
            else:
                cur_pos = position; cur_ep = entry_price
            if cur_pos and cur_ep > 0:
                ...
                if pr <= -0.0005: should_close = True; close_reason = "1%硬止损"
        ...
        if data.get("e") == "kline":
            ...
            if k_data["x"]:
                ...
                if k_data["i"] == "1m":
                    ...
                    if len(kline_1m_closed) > 15: kline_1m_closed.pop(0)  # 扩大缓存以支持KDJ
                    # 修复1：1分钟K线收盘时计算KDJ
                    if len(kline_1m_closed) >= 9:
                        closes = [k['close'] for k in kline_1m_closed]
                        compute_kdj(closes)
...
if __name__ == "__main__":
    ...
    log(f"        策略: 拐点回调 + 顺势追单(15m过滤) | 仓位: 总资金 {POSITION_RATIO * 100:.0f}%")
    log("======================================")
    sync_binance_time()
