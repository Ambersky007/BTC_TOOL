...
COOLDOWN_SEC = 2
# V23 核心参数
SCALP_SL = 0.025  # 2.5% 动态止损验证区
ABSOLUTE_HARD_SL = 0.05  # 🚀 新增：5% 绝对硬止损红线，无视任何拦截
...
recent_agg_trade_vols = []  # 🚀 新增：记录近期逐笔真实成交量，用于量价共振判断
entry_atr = 0.0  # 🚀 新增：记录开仓那一刻的ATR，作为止损锚定基准
prev_orderbook_snapshot = {"bid_vol": 0.0, "ask_vol": 0.0, "time": 0.0}  # 🚀 新增：盘口快照，用于算闪烁率
...
def check_order_filled():
    ...
    global local_max_profit_pct, local_max_price, local_min_price
    global entry_time, time_at_max_profit, entry_atr
    ...
    if USE_LOCAL_SIMULATION:
        ...
        if filled:
            if is_reduce:
                ...
                log(f"🎉 [本地限价单成交] {side} | 成交价:{order_p} | 数量:{actual_position_amt} | 锁定ATR:{entry_atr:.1f}")
        return
    if not active_order: return
    try:
        p = {"symbol": SYMBOL, "orderId": active_order, "timestamp": get_timestamp(), "recvWindow": 5000}
        ...
        if d.get("status") == "FILLED":
            ...
            if is_reduce:
                ...
                with data_lock:
                    entry_atr = compute_atr(kline_1m_closed, 7) if len(kline_1m_closed) >= 7 else 0.0
                sync_position()
                log(f"🎉 [API限价单成交] {d.get('side')} | 成交价:{d.get('avgPrice')} | 锁定ATR:{entry_atr:.1f}")
...
# ===================== 🚀 新增：盘口闪烁与做市商识别 🚀 =====================
def get_spoofing_features():
    """
    获取盘口闪烁特征（深度7档）
    """
    global prev_orderbook_snapshot
    try:
        bids = orderbook.get("bids", [])[:7]
        asks = orderbook.get("asks", [])[:7]
        if not bids or not asks: return 0.0, 0.0, False, False, False
        bid_vol = sum(float(b[1]) for b in bids)
        ask_vol = sum(float(a[1]) for a in asks)
        now = time.time()
        bid_change = abs(bid_vol - prev_orderbook_snapshot["bid_vol"])
        ask_change = abs(ask_vol - prev_orderbook_snapshot["ask_vol"])
        # 至少间隔0.5秒才计算闪动，避免同帧数据误判
        if now - prev_orderbook_snapshot["time"] >= 0.5:
            prev_orderbook_snapshot.update({"bid_vol": bid_vol, "ask_vol": ask_vol, "time": now})
            flash_bid = bid_change > bid_vol * 0.4
            flash_ask = ask_change > ask_vol * 0.4
        else:
            flash_bid = False
            flash_ask = False
        # 成交量是否同步放大
        with data_lock:
            if len(kline_1m_closed) >= 3:
                vols = [k['volume'] for k in kline_1m_closed[-3:]]
                vol_spike = vols[-1] > sum(vols) / 3 * 1.5
            else:
                vol_spike = False
        return bid_vol, ask_vol, flash_bid, flash_ask, vol_spike
    except:
        return 0.0, 0.0, False, False, False


def detect_strong_reverse(atr):
    """
    检测任意方向的量价共振（双向封锁开仓）
    """
    with data_lock:
        if len(kline_1m_closed) < 3 or atr <= 0: return False
        recent = kline_1m_closed[-3:]
        vols = [k['volume'] for k in recent]
        bodies = [abs(k['close'] - k['open']) for k in recent]
        vol_increase = vols[0] < vols[1] < vols[2]
        strong_body = all(b > atr * 0.5 for b in bodies)
        # 价格连续推进 (不管多空，只要连续推进就是趋势，不接飞刀)
        bullish_confirm = all(recent[i]['close'] > recent[i - 1]['close'] for i in range(1, 3))
        bearish_confirm = all(recent[i]['close'] < recent[i - 1]['close'] for i in range(1, 3))
        price_confirm = bullish_confirm or bearish_confirm
        return vol_increase and strong_body and price_confirm


def entry_direction_filter(atr):
    """
    开仓方向过滤器：双向封锁量价共振，幽灵单反向指引，做市商控盘加仓
    返回: (allow_long, allow_short, notional_multiplier, spoof_type)
    """
    # 1. 量价共振双向封锁
    if detect_strong_reverse(atr):
        return False, False, 1.0, "none"
    bid_vol, ask_vol, flash_bid, flash_ask, vol_spike = get_spoofing_features()
    both_large = max(bid_vol, ask_vol) > 0 and (max(bid_vol, ask_vol) / min(bid_vol, ask_vol) < 2)
    # 2. 做市商控盘识别（双边闪+不放量）
    if both_large and flash_bid and flash_ask and not vol_spike:
        return True, True, 1.5, "market_making"
    # 3. 单边幽灵单指引
    # 卖盘挂大幽灵单(假压盘) -> 主力吸筹做多 -> 允许多，禁止空
    if ask_vol > bid_vol * 2 and flash_ask and not vol_spike:
        return True, False, 1.0, "ask_spoof"
    # 买盘挂大幽灵单(假托盘) -> 主力出货做空 -> 允许空，禁止多
    if bid_vol > ask_vol * 2 and flash_bid and not vol_spike:
        return False, True, 1.0, "bid_spoof"
    return True, True, 1.0, "normal"
...
def scalp_entry():
    ...
    if current_zone_score >= 70:
        ...
        return
    # 🚀 注入开仓方向过滤器 🚀
    allow_long, allow_short, filter_multiplier, spoof_type = entry_direction_filter(atr)
    notional_multiplier = min(notional_multiplier * filter_multiplier, 1.5)  # 乘积不能超过1.5
    if spoof_type == "market_making":
        strategy_suffix += "+做市商控盘"
    elif spoof_type == "ask_spoof":
        strategy_suffix += "+卖盘诱导"
    elif spoof_type == "bid_spoof":
        strategy_suffix += "+买盘诱导"
    ...
    if price <= zone.low + PRICE_STEP * 5:
        if zone.touch_count >= 0 and (
            ...
            if long_allowed and allow_long:  # 🚀 加入过滤器许可
    ...
    if price >= zone.high - PRICE_STEP * 5:
        if zone.touch_count >= 0 and (bias != "bull" or detect_absorption() or exhaustion) and last_trade_side != "buy":
            if short_allowed and allow_short:  # 🚀 加入过滤器许可
...
# 🚀🚀🚀 新增：止损激活过滤器（进阶版 - 锚定ATR + 方向波动率 + 闪烁幽灵单） 🚀🚀🚀
def stop_activation_filter(direction, current_atr):
    """
    止损激活过滤器
    返回: True (触发止损), False (拦截止损)
    """
    with data_lock:
        if len(kline_1m_closed) < 3:
            return True
        ...
        # 1. 波动率扩张检测（锚定开仓ATR）
        # ======================
        real_time_high = max(last_k['high'], raw_price if raw_price else 0)
        real_time_low = min(last_k['low'], raw_price if raw_price else 999999)
        real_time_range = real_time_high - real_time_low
        # 使用开仓锚定ATR，如果没记录上则降级用当前ATR
        base_atr = entry_atr if entry_atr > 0 else current_atr
        is_vol_expansion = False
        if base_atr > 0 and real_time_range > base_atr * 2.5:
            is_vol_expansion = True
        if is_vol_expansion:
            # 判断方向：反向扩张止损，正向扩张拿稳
            is_bearish = (raw_price < last_k['open']) if raw_price else (last_k['close'] < last_k['open'])
            is_bullish = (raw_price > last_k['open']) if raw_price else (last_k['close'] > last_k['open'])
            if direction == "long" and is_bearish:
                log(f"⚠️ [反向波动扩张] 多单遭遇暴跌! 实时波幅:{real_time_range:.1f} > 锚定ATR*2.5:{base_atr * 2.5:.1f} -> 触发止损")
                return True
            elif direction == "short" and is_bullish:
                log(f"⚠️ [反向波动扩张] 空单遭遇暴涨! 实时波幅:{real_time_range:.1f} > 锚定ATR*2.5:{base_atr * 2.5:.1f} -> 触发止损")
                return True
            elif direction == "long" and is_bullish:
                log(f"🚀 [正向波动扩张] 多单遇主升浪发飙! 拿稳等盈利扩大")
                return False
            elif direction == "short" and is_bearish:
                log(f"🚀 [正向波动扩张] 空单遇主跌浪发飙! 拿稳等盈利扩大")
                return False
    # ======================
    # 2. 幽灵单检测（闪烁率 + 方向分类）
    # ======================
    bid_vol, ask_vol, flash_bid, flash_ask, vol_spike = get_spoofing_features()
    if direction == "long":
        # 反向幽灵单（上方假压盘，主力诱导） -> 拦截止损
        if ask_vol > bid_vol * 2 and flash_ask and not vol_spike:
            log(f"[幽灵单护盘] 检测到反向巨量挂单闪烁，拦截止损拿稳！")
            return False
        # 正向幽灵单（下方假托盘，主力出货） -> 触发止损
        if bid_vol > ask_vol * 2 and flash_bid and not vol_spike:
            log(f"[幽灵单警报] 检测到正向巨量挂单闪烁，主力诱多出货，提前触发止损！")
            return True
    elif direction == "short":
        # 反向幽灵单（下方假托盘，主力诱导） -> 拦截止损
        if bid_vol > ask_vol * 2 and flash_bid and not vol_spike:
            log(f"[幽灵单护盘] 检测到反向巨量挂单闪烁，拦截止损拿稳！")
            return False
        # 正向幽灵单（上方假压盘，主力诱空出货） -> 触发止损
        if ask_vol > bid_vol * 2 and flash_ask and not vol_spike:
            log(f"[幽灵单警报] 检测到正向巨量挂单闪烁，主力诱空出货，提前触发止损！")
            return True
    # ======================
    # 3. 常规量价确认 + 价格推进
    # ======================
    with data_lock:
        ...
        vol_increase = vols[0] < vols[1] < vols[2]
        bodies = [abs(k['close'] - k['open']) for k in recent]
        strong_body = all(b > current_atr * 0.5 for b in bodies) if current_atr > 0 else False
        if direction == "long":
            price_confirm = all(recent[i]['close'] < recent[i - 1]['close'] for i in range(1, 3))
            trend_confirm = all(k['close'] < k['open'] for k in recent)
        else:
            price_confirm = all(recent[i]['close'] > recent[i - 1]['close'] for i in range(1, 3))
            trend_confirm = all(k['close'] > k['open'] for k in recent)
    activation = (vol_increase and strong_body and trend_confirm and price_confirm)
    return activation
...
def detect_aggressive_reversal():
    ...
    is_vol_surge = False
    with data_lock:
        if len(recent_agg_trade_vols) >= 5:
            hist_vols = recent_agg_trade_vols[:-3] if len(recent_agg_trade_vols) > 3 else recent_agg_trade_vols
            avg_vol = sum(hist_vols) / len(hist_vols) if hist_vols else 1.0
            recent_max_vol = max(recent_agg_trade_vols[-3:])
            if recent_max_vol > avg_vol * 5:
                is_vol_surge = True
    if reverse_ticks >= 8 and is_vol_surge:
        log(f"[提前止损] 检测到量价共振反转：反向Tick:{reverse_ticks} | 放量:{is_vol_surge}")
        return True
    return False
...
def manage_position():
    if USE_LOCAL_SIMULATION:
        ...
        if ep > 0:
            ...
            # 🚀 绝对红线 -5% 判定 🚀
            if pnl <= -ABSOLUTE_HARD_SL:
                close_position(f"绝对红线硬止损({ABSOLUTE_HARD_SL * 100}%)")
                return True
            # 🚀 动态验证区 -2.5% 判定 🚀
            elif pnl <= -SCALP_SL:
                with data_lock:
                    atr = compute_atr(kline_1m_closed, 7) if len(kline_1m_closed) >= 7 else 0
                if atr <= 0 or stop_activation_filter(pos, atr):
                    close_position("V23动态止损(验证通过)")
                    return True
                else:
                    log("[动态止损被拦截] 噪音扫损/幽灵单护盘(本地)")
    else:
        with lock:
            pos = position;
            off_pnl = official_unrealized_profit;
        ...
        if off_margin > 0:
            ...
            # 🚀 动态验证区 -2.5% 判定 🚀
                ...
                with data_lock:
                    atr = compute_atr(kline_1m_closed, 7) if len(kline_1m_closed) >= 7 else 0
                if atr <= 0 or stop_activation_filter(pos, atr):
                    close_position("V23动态止损(官方盈亏触发)")
                    return True
                else:
                    log("[动态止损被拦截] 噪音扫损/幽灵单护盘(官方)")
        ...
        if not cur_pos or cur_ep <= 0:
            ...
            if USE_LOCAL_SIMULATION:
                ...
                # 🚀 动态验证区 -2.5% 判定 🚀
                    ...
                    with data_lock:
                        atr = compute_atr(kline_1m_closed, 7) if len(kline_1m_closed) >= 7 else 0
                    if atr <= 0 or stop_activation_filter(cur_pos, atr):
                        close_position(f"动态止损({SCALP_SL * 100}%)")
                        return
                    else:
                        log("[动态止损被拦截] 噪音扫损(process_price)")
                if cur_pos == "long":
    ...
    try:
        ...
        if data.get("e") == "aggTrade":
            ...
            with data_lock:
                ...
                recent_agg_trade_vols.append(trade_qty)
                if len(recent_agg_trade_vols) > 50: recent_agg_trade_vols.pop(0)
            trade_queue.put("PROCESS")
        if data.get("e") == "kline":
            k_data = data["k"]
            if k_data["x"]:
        ...
        try:
            ...
            if data.get("e") == "ACCOUNT_UPDATE":
                ...
                if o.get("s") == SYMBOL and o.get("X") == "FILLED":
                    ...
                    # 🚀 锁定实盘开仓的ATR基准 🚀
                    if not o.get("rd"):  # 如果不是平仓单
                        with data_lock:
                            entry_atr = compute_atr(kline_1m_closed, 7) if len(kline_1m_closed) >= 7 else 0.0
                        log(f"[实盘ATR锁定] 锚定ATR:{entry_atr:.1f}")
        except Exception as e:
...
if __name__ == "__main__":
    ...
    log(f" 策略: 流动性+Sweep+区间评分 | 启动盈利:0.01% | 止损:{SCALP_SL * 100}% | 绝对红线:{ABSOLUTE_HARD_SL * 100}% | 区间有效期:动态ATR自适应")
    log("======================================")
    sync_binance_time()
